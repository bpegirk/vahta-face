...
const (
	EventTurniketAllow     = 21
	EventCardDisabled      = 52
	EventDeny              = 54
	EventDenyDoubleControl = 58
	EventCardReader        = 33
)
...
func ParseBody(body []byte) {
	...
	allowEvents := []int{EventTurniketAllow, EventCardReader, EventCardDisabled, EventDenyDoubleControl, EventDeny}
	if indexOf(command, allowEvents) != -1 {
		...
		if pOffset > 0 {
			...
			if userId > 0 {
				fio, photo := PeopleName(int(userId))
...
func getHwOffset(eventId int) int {
	switch eventId {
	case EventCardReader, EventCardDisabled, EventDenyDoubleControl, EventDeny:
		return 9
	case EventTurniketAllow:
		return 13
	}
	return 0
}
